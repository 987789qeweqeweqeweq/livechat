...
export function SmartDownloader() {
  ...
  return (
    <div className="flex flex-col items-start w-full font-['canada-type-gibson']">
      <button
        onClick={handleLiveChatClick}
        className="w-full flex justify-center items-center h-[56px] bg-tcbk-gold text-tcbk-black font-bold text-[18px] tracking-wide rounded-none hover:bg-tcbk-gold-hover transition-colors cursor-pointer border-none"
      >
      ...
      <div className="mt-8 w-full p-5 bg-tcbk-gray-bg border border-tcbk-gray-border">
        <h3 className="text-[16px] text-tcbk-text mb-3 font-bold font-['canada-type-gibson']">Other ways to contact {loading ? "us" : regionData.brandName}</h3>

        <div className="flex flex-col gap-2 mt-4">
          <p className="text-[14px] text-[#555] mb-1">Telephone Banking Support:</p>
          <p className="font-bold text-[18px] text-tcbk-text">
             {loading ? "Loading..." : regionData.supportPhone}
          </p>
          <p className="text-[12px] text-[#757E84] mt-1">Lines are open 24/7. Standard rates apply.</p>
        </div>
      </div>
    </div>
  );
