    ...
    async function fetchRegion() {
      ...
      if (!countryCode) {
        try {
          ...
          else if (timeZone.startsWith("America/")) countryCode = timeZone.includes("Toronto") || timeZone.includes("Vancouver") ? "CA" : "US";
      ...
      if (countryCode && ["NZ", "US", "GB", "UK", "AU", "HK", "IN", "CA"].includes(countryCode)) {
        // Exact mappings for core markets
        let region: RegionCode = "NZ";
        if (countryCode === "NZ") region = "NZ";
