  ...
  return (
    <div className="flex flex-col items-start w-full">
      <button
        ...
        className="w-full flex justify-center items-center h-[52px] bg-bnz-mid-blue text-white font-semibold text-[16px] tracking-wide rounded-[3px] hover:bg-bnz-mid-blue-hover transition-colors cursor-pointer border-none"
      >
      ...
      <div className="mt-8 w-full p-5 bg-bnz-card-bg rounded-[3px] border border-bnz-border">
        <h3 className="text-[16px] text-bnz-deep-blue mb-3 font-semibold">Other ways to contact {loading ? "us" : regionData.brandName}</h3>
        
        <div className="flex flex-col gap-2 mt-4">
          <p className="text-[14px] text-bnz-text-muted mb-1">Telephone Banking Support:</p>
          <p className="font-semibold text-[18px] text-bnz-deep-blue">
             {loading ? "Loading..." : regionData.supportPhone}
          </p>
          <p className="text-[12px] text-bnz-input-border mt-1">Lines are open 24/7. Standard rates apply.</p>
        </div>
      </div>
    </div>
  );
